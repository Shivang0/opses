...
/**
 * Table primitives — clean, dense, accessible <table> semantics.
 * The root wraps the table in a horizontally scrollable region so wide
...
export const TR = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn(
        'transition-colors hover:bg-canvas data-[state=selected]:bg-primary-soft',
        className,
      )}
      {...props}
    />
...
>(({ className, scope = 'col', ...props }, ref) => (
  <th
    ...
    scope={scope}
    className={cn(
      'h-10 px-3 text-left align-middle text-xs font-semibold uppercase tracking-wide text-muted whitespace-nowrap',
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <td ref={ref} className={cn('px-3 py-2.5 align-middle text-ink-soft', className)} {...props} />
))
TD.displayName = 'TD'
